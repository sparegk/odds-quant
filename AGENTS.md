...
Import historical results with `python -m app.cli import-results path/to/results.csv`, train with `python -m app.cli train-poisson ...`, persist a pre-kickoff prediction with `python -m app.cli predict-event ...`, replay a model specification with `python -m app.cli evaluate-model ...`, and generate gated signals with `python -m app.cli generate-signals ...`. Model versions, evaluations, and signals must retain their exact cutoff, feature version, input fingerprint, sample size, evaluation status, and price provenance.
