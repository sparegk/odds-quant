...
export function Overview({ dashboard, onSelectEvent, onNavigate }: { dashboard: DashboardData; onSelectEvent: (eventId: number) => void; onNavigate: (view: ViewKey) => void }) {
  ...
  if ((readiness?.non_demo_probability_validated_evaluations ?? dashboard.evaluations.filter((run) => !run.is_demo && run.probability_evaluation_status === 'probability_validated').length) === 0) actions.push({ title: 'Validate probability research', detail: 'A stored evaluation is not enough; the model must pass the independent probability policy on untouched chronological evidence.', target: 'models', targetLabel: 'Model performance' })
  if ((readiness?.non_demo_calibrated_evaluations ?? dashboard.evaluations.filter((run) => !run.is_demo && run.evaluation_status === 'calibrated').length) === 0) actions.push({ title: 'Establish market/value validation', detail: 'Value and underdog outputs remain blocked without qualifying historical market evidence.', target: 'models', targetLabel: 'Model performance' })
