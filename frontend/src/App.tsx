...
export function Methodology() {
  ...
  return (
    <div className="max-w-5xl">
      ...
      <MethodologyLifecycle lifecycle={lifecycle} />
...
function MethodologyLifecycle({ lifecycle }: { lifecycle: string[][] }) {
  return <section className="mt-7" aria-label="Operational evidence lifecycle">
    <SectionHeading eyebrow="Point-in-time controls" title="Operational evidence lifecycle" />
    <div className="overflow-x-auto border-y border-zinc-200 bg-white"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-zinc-50 text-xs uppercase text-zinc-500"><tr><th className="px-4 py-3">Step</th><th className="px-4 py-3">Layer</th><th className="px-4 py-3">Stored evidence</th><th className="px-4 py-3">Fail-closed boundary</th></tr></thead><tbody>{lifecycle.map(([step, layer, evidence, boundary]) => <tr className="border-t border-zinc-100 align-top" key={step}><td className="px-4 py-3 font-mono font-bold">{step}</td><td className="px-4 py-3 font-bold">{layer}</td><td className="px-4 py-3 text-zinc-600">{evidence}</td><td className="px-4 py-3 text-amber-800">{boundary}</td></tr>)}</tbody></table></div>
  </section>
}
