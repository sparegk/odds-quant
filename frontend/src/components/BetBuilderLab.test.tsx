...
describe('BuilderQuoteCard', () => {
  it('separates joint probability from the naive product and labels demo provenance', () => {
    ...
    expect(screen.getByText('+6.5 pp')).toBeInTheDocument()
    expect(screen.getByText('1.236x')).toBeInTheDocument()
    ...
    expect(screen.getByLabelText('Builder settlement checklist')).toHaveTextContent('Scoreline-derived legsVALIDATED')
    expect(screen.getByLabelText('Builder settlement checklist')).toHaveTextContent('Bookmaker termsMANUAL CHECK')
    expect(screen.getByText(/Player props/).parentElement).toHaveTextContent('BLOCKED')
  })
})
