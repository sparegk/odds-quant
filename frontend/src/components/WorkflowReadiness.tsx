...
const requirementsByView: Record<string, Requirement[]> = {
  overview: [
    ...
    req('non_demo_probability_validated_evaluations', 'Validate probabilities', 'Run a qualifying probability evaluation', 'models'),
    req('non_demo_calibrated_evaluations', 'Validate market value', 'Add qualifying historical market evidence', 'models'),
...
export function WorkflowReadiness({ dashboard, view, onNavigate }: { dashboard: DashboardData; view: string; onNavigate: (target: string) => void }) {
  ...
  return <section className={`mb-6 border-l-4 px-4 py-4 ${blocked.length ? 'border-amber-400 bg-amber-50' : 'border-emerald-500 bg-emerald-50'}`} aria-label="Workflow readiness"><div className="flex items-start gap-3">{blocked.length ? <CircleAlert aria-hidden="true" className="mt-0.5 shrink-0 text-amber-800" size={19} /> : <CheckCircle2 aria-hidden="true" className="mt-0.5 shrink-0 text-emerald-800" size={19} />}<div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-6"><div><p className="font-bold">{isOverview ? 'Research workflow' : blocked.length ? `${blocked.length} prerequisite${blocked.length === 1 ? '' : 's'} missing` : 'Workflow prerequisites available'}</p>{isOverview ? <p className="mt-1 text-xs leading-5 text-zinc-600">Complete each timestamp-safe stage in order. The first incomplete stage is always the recommended next action.</p> : null}</div>{isOverview && next ? <button className="inline-flex shrink-0 items-center gap-2 rounded-[5px] bg-zinc-900 px-3 py-2 text-xs font-bold text-white hover:bg-zinc-700" onClick={() => onNavigate(next.target)} type="button">Next: {next.label}<ArrowRight aria-hidden="true" size={14} /></button> : null}</div><ol className={`mt-3 grid gap-2 ${isOverview ? 'grid-cols-8' : 'grid-cols-3'}`}>{requirements.map((item, index) => { const ready = counts[item.key] > 0; const current = item === next; return <li aria-current={current ? 'step' : undefined} key={item.key} className={`flex min-h-20 items-start justify-between gap-3 border px-3 py-2 text-xs ${current ? 'border-amber-500 bg-white shadow-sm' : ready ? 'border-emerald-200 bg-emerald-50/70' : 'border-zinc-200 bg-white/60'}`}><div><div className="mb-2 flex items-center gap-2"><span className={`grid h-5 w-5 place-items-center rounded-full text-[10px] font-bold ${ready ? 'bg-emerald-700 text-white' : current ? 'bg-amber-500 text-zinc-950' : 'bg-zinc-200 text-zinc-600'}`}>{ready ? '✓' : index + 1}</span>{current ? <span className="font-bold uppercase text-amber-800">Next</span> : ready ? <span className="font-bold uppercase text-emerald-800">{readinessStateLabel(item.key)}</span> : <span className="font-bold uppercase text-zinc-400">Blocked</span>}</div><p className="font-bold">{item.label}</p><p className={ready ? 'mt-1 text-emerald-700' : current ? 'mt-1 text-amber-800' : 'mt-1 text-zinc-500'}>{ready ? `${counts[item.key]} ${readinessCountLabel(item.key)}` : item.action}</p></div>{!ready && !isOverview ? <button aria-label={`Go to ${item.action}`} className="grid h-7 w-7 shrink-0 place-items-center rounded-[4px] border border-zinc-300 bg-white hover:border-zinc-500" onClick={() => onNavigate(item.target)} type="button"><ArrowRight aria-hidden="true" size={14} /></button> : null}</li> })}</ol>{blocked.length && !isOverview ? <p className="mt-3 text-xs leading-5 text-amber-900">This screen remains usable for inspection, but evidence-dependent outputs stay blocked until every required layer is stored.</p> : null}{isOverview && !blocked.length ? <p className="mt-3 text-xs font-semibold text-emerald-900">The complete research chain is available for inspection. Re-run stages only when new timestamped evidence is imported.</p> : null}</div></div></section>
}

function signalRequirements(): Requirement[] { return [req('odds_snapshots', 'Compatible odds', 'Import complete odds snapshots', 'data'), req('model_versions', 'Trained model', 'Train a model version', 'models'), req('non_demo_probability_validated_evaluations', 'Probability validation', 'Run a qualifying probability evaluation', 'models'), req('non_demo_calibrated_evaluations', 'Market/value validation', 'Add qualifying historical market evidence', 'models'), req('predictions', 'Stored predictions', 'Persist pre-kickoff predictions', 'models'), req('signals', 'Generated signals', 'Generate signals from predictions', 'models')] }
...
function isQualificationKey(key: keyof ReadinessCounts): boolean { return key === 'non_demo_probability_validated_evaluations' || key === 'non_demo_calibrated_evaluations' }
function readinessStateLabel(key: keyof ReadinessCounts): string { return isQualificationKey(key) ? 'Qualified' : 'Stored' }
function readinessCountLabel(key: keyof ReadinessCounts): string { return isQualificationKey(key) ? 'qualified' : 'stored' }
function fallbackCounts(dashboard: DashboardData): ReadinessCounts { return { events: dashboard.events.length, odds_snapshots: dashboard.providers.reduce((sum, provider) => sum + provider.snapshot_count, 0), final_results: 0, model_versions: dashboard.models.length, predictions: 0, non_demo_probability_validated_evaluations: dashboard.evaluations.filter((run) => !run.is_demo && run.probability_evaluation_status === 'probability_validated').length, non_demo_calibrated_evaluations: dashboard.evaluations.filter((run) => !run.is_demo && run.evaluation_status === 'calibrated').length, signals: dashboard.signals.length, signal_backtests: dashboard.backtests.length, bookmaker_tax_mappings: 0, bookmaker_constraints: 0, intelligence_records: 0 } }
