...
export function DataOperations({ dashboard, onChanged }: { dashboard: DashboardData; onChanged?: () => Promise<void> | void }) {
  ...
  return <div className="space-y-8">
    ...
    <section><Heading eyebrow="Sources" title="Provider status" />{dashboard.providers.length ? <DesktopDataTable ariaLabel="Provider status" columns={providerColumns} filename="provider-status.csv" rowKey={(provider) => provider.id} rows={dashboard.providers} /> : <Empty text="No providers registered." />}</section>
    <section className="grid gap-7 xl:grid-cols-2"><div><Heading eyebrow="Ingestion" title="Recent import jobs" />{dashboard.imports.length ? <div className="border-y border-zinc-200 bg-white">{dashboard.imports.map((job) => <details key={job.id} className="border-b border-zinc-100 px-4 py-3 last:border-0"><summary className="cursor-pointer list-none"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">#{job.id} · {job.filename}</p><p className="mt-1 text-xs text-zinc-500">{job.rows_imported}/{job.rows_received} rows · {formatDateTime(job.created_at)}</p></div><Badge status={job.status} /></div></summary>{job.errors.length ? <div className="mt-3 border-t border-zinc-100 pt-3"><div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase text-rose-700">Rejection details</p><RejectionReportLink job={job} /></div>{job.errors.map((error, index) => <pre key={index} className="mt-2 overflow-x-auto whitespace-pre-wrap bg-rose-50 p-3 text-xs text-rose-900">{JSON.stringify(error, null, 2)}</pre>)}</div> : <p className="mt-3 border-t border-zinc-100 pt-3 text-xs text-zinc-500">No row errors recorded.</p>}</details>)}</div> : <Empty text="No import jobs recorded." />}</div><div><Heading eyebrow="Scheduler" title="Provider jobs" />{dashboard.jobs.length ? <div className="border-y border-zinc-200 bg-white">{dashboard.jobs.map((job) => <div key={job.id} className="flex items-start justify-between gap-3 border-b border-zinc-100 px-4 py-3 last:border-0"><div><p className="text-sm font-semibold">{job.provider} · {humanizeCode(job.job_type)}</p><p className="mt-1 text-xs text-zinc-500">{job.message || 'No provider message'} · {formatDateTime(job.created_at)}</p>{job.finished_at ? <p className="mt-1 text-xs text-zinc-400">Finished {formatDateTime(job.finished_at)}</p> : null}</div><Badge status={job.status} /></div>)}</div> : <Empty text="No provider jobs recorded." />}</div></section>
...
const providerColumns: DesktopColumn<DashboardData['providers'][number]>[] = [
  { id: 'provider', label: 'Provider', value: (provider) => provider.name + ' ' + provider.slug, render: (provider) => <><p className="font-semibold">{provider.name}</p><p className="text-xs text-zinc-500">{provider.slug}</p></> },
  { id: 'kind', label: 'Kind', value: (provider) => humanizeCode(provider.kind) },
  { id: 'events', label: 'Events', value: (provider) => provider.event_count, align: 'right' },
  { id: 'snapshots', label: 'Snapshots', value: (provider) => provider.snapshot_count, align: 'right' },
  { id: 'classification', label: 'Classification', value: (provider) => provider.is_demo ? 'demo' : 'external', render: (provider) => <Badge status={provider.is_demo ? 'demo' : 'external'} /> },
]
