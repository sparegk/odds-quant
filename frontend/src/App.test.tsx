...
describe('BacktestResearch', () => {
  it('shows settled return evidence separately from calibration', () => {
    ...
    expect(screen.getAllByText('Research Only')).toHaveLength(2)
    expect(screen.getAllByText('+13.3%')).toHaveLength(2)
    ...
    expect(screen.getByRole('heading', { name: 'Backtest sufficiency audit' })).toBeInTheDocument()
    expect(screen.getByText('Run evidence remains incomplete')).toBeInTheDocument()
    expect(screen.getByText('1 detailed observation for 24 reported bets.')).toBeInTheDocument()
    expect(screen.getByText(/not evidence of repeatable profitability/)).toBeInTheDocument()
  })
})
