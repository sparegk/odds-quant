...
- Completed: fingerprint-bound pre-registered external-validation receipts in the evaluation API and Model Performance, including examined/no-retuning and separate market-authorization states.
- Development evidence: an opt-in league-prior Poisson cold-start benchmark recovered full 279-event Bundesliga coverage without altering any strict-eligible forecast, but its 0.08225 ECE still missed policy. It remains inactive pending a frozen specification and genuinely new untouched holdout.
- Completed: an exact-window model experiment matrix comparing primary Poisson/Elo runs with aligned Dixon-Coles, nested-selection, and chronological-ensemble evidence, including observation alignment, configuration provenance, and paired-interval verdicts.
...
- External-validation replay completed: `BUNDESLIGA_EXTERNAL_VALIDATION.md` records the frozen CC0 Bundesliga experiment and immutable run receipt. The run retained 219 of 279 candidates (78.49%), missed the fixed ECE and coverage gates, and did not establish superiority to uniform at the paired 95% level. Its status is `insufficient_evidence`; the examined holdout cannot be retuned or relabelled as untouched.
