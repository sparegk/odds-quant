...
The predictive data flow is: permitted source or user CSV -> normalized ingestion DTO -> immutable raw snapshot -> timestamped result and odds tables -> cutoff-filtered training rows -> fingerprinted Poisson model version -> pre-kickoff score matrix and selection predictions -> expanding-window replay -> immutable per-event scores and calibration buckets -> calibrated prediction plus fresh compatible market consensus -> persisted explainable signal. Signals fail closed for demo evidence, post-cutoff calibration, stale best prices, material movement, weak uncertainty bounds, or incomplete markets.
