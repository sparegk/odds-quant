...
def settle(
    home_goals: int | None, away_goals: int | None, market_type: str, code: str, line: float | None
) -> Settlement:
    ...
    if (
        line is not None
        and line.is_integer()
        and market_type in {"TOTAL_GOALS", "HOME_TEAM_TOTAL", "AWAY_TEAM_TOTAL"}
    ):
