...
def test_pamestoixima_coverage_cli_is_machine_actionable(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    result = PamestoiximaEdgeCoverageView(
        contract_version="cold-start-v2-pamestoixima-edge-validation-v1",
        cohort_selection_id="premier-league-2026-27-post-pamestoixima-freeze-full-season",
        ...
        final_result_events=0,
        prediction_events=1,
        permitted_snapshots=1770,
        permitted_snapshot_events=10,
        decision_window_events=0,
        explicit_closing_events=0,
        cost_profile_events=0,
        decision_window_coverage=0,
        closing_coverage=0,
        cost_profile_coverage=0,
        minimum_observations=160,
        minimum_candidate_coverage=0.8,
        minimum_closing_coverage=0.8,
        market_consensus_authorized=False,
        acquisition_ready=False,
        replay_authorized=False,
        blockers=["incomplete_candidate_universe"],
    )
    monkeypatch.setattr(
        sys,
        "argv",
        ["odds-quant", "audit-pamestoixima-edge-coverage", "--fail-on-blockers"],
    )
    monkeypatch.setattr(cli, "SessionLocal", _SessionContext)
    monkeypatch.setattr(cli, "pamestoixima_edge_coverage", lambda *args, **kwargs: result)

    assert cli.main() == 4
    payload = json.loads(capsys.readouterr().out)
    assert payload["market_consensus_authorized"] is False
    assert payload["blockers"] == ["incomplete_candidate_universe"]
    assert "two_bookmaker_events" not in payload
    assert "roi" not in payload
    assert "clv" not in payload
