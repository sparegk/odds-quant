...
These market calculations are deliberately not labelled `VALUE`: expected value and probability edge require an independently trained, timestamped model prediction. The React dashboard consumes the stored-data API and blocks model-dependent views until those records exist. The next slice will connect modelling, signals, underdogs, arbitrage, and backtesting. No model performance or profitability result is claimed by the seed data.
