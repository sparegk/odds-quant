...
Market calculations are deliberately not labelled `VALUE`: the next slice must perform chronological held-out calibration and then join an independently evaluated prediction to a fresh compatible price. Signals, underdogs, arbitrage, and backtesting remain unimplemented. Synthetic history validates software behavior only and no model performance or profitability result is claimed.
