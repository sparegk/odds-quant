...
## Matchday Research Experience

Matchday is the primary fixture-first workflow. It groups imported matches by the user's local calendar day and prioritizes the Premier League, Bundesliga, Ligue 1, La Liga, UEFA Champions League, UEFA Europa League, UEFA Conference League, top domestic cups, and major international events. The interaction follows the useful schedule-to-match-detail pattern seen in score and statistics products such as [FotMob](https://www.fotmob.com/en/download), while preserving OddsQuant's stricter point-in-time evidence rules.

Opening a match joins only records available before kickoff: recent final results for both teams, the latest coherent bookmaker snapshots, the latest model output, qualified value signals, and stored bet-builder quotes. Team form is a compact descriptive input, not a betting recommendation. Likely outcomes are shown separately from value; a bet candidate requires a calibrated signal tied to an offered price. Best bookmaker is selection-specific because one operator is rarely best for every outcome, and parlays are comparable only when every leg, line, period, and settlement rule is identical.

Player and tactical sections fail closed. Stored player statistics or a lineup alone do not authorize a player bet. Player markets remain blocked until player-level targets, availability history, position-appropriate features, settlement rules, and chronological calibration are independently validated. The same principle applies to bet builders: a plausible or high-probability combination is not labelled value without an identical timestamped offered quote whose lower probability bound produces positive expected value.
...
The backtesting layer reports predictive calibration and stored-signal strategy metrics, including Brier score, log loss, ECE, ROI, yield, profit in units, and drawdown. CLV coverage remains zero unless timestamped closing prices were stored. Synthetic results demonstrate software behavior only.

## Current Status

The data-foundation, model-baseline, and research-workflow portions of Phase 1 are operational. Alembic creates and evolves the point-in-time schema. The backend has strict odds and historical-result CSV contracts, atomic ingestion, immutable raw import records, rejected-job audits, timestamped result corrections, and deterministic current-anchor synthetic odds and history seeds.

Versioned routes now expose events, event detail, providers, import jobs, multipart odds import, stored odds comparison, local-date matchdays, and matchday research detail. Matchday responses group featured competitions and enforce a pre-kickoff cutoff for form, odds, predictions, signals, and builder evidence. Comparison responses calculate raw implied probability, bookmaker overround and margin, proportional and power de-vig probabilities, corresponding fair odds, freshness, and best available price from each bookmaker's latest coherent snapshot as of the requested time.

The backend trains a shrunk venue-specific Poisson team-strength baseline using only final result observations available by the requested cutoff. Each immutable model version stores its training window, input fingerprint, feature version, parameters, sample size, and `unvalidated` evaluation status. Pre-kickoff predictions persist expected goals, the score matrix, canonical derived markets, selection probabilities, sampling intervals, and exact input timestamp. The dashboard exposes stored evidence while continuing to block unsupported value and performance claims.

The backend now performs expanding-window held-out replay with exact per-event training fingerprints, proper 1X2 scores, calibration buckets, eligibility coverage, uniform and compatible point-in-time market benchmarks, and deterministic moving-block bootstrap intervals. Paired loss differences are computed only on aligned events, and non-demo promotion requires both upper 95% differences against uniform to be negative. The dashboard exposes these runs without converting synthetic results into evidence.

Market comparison is never labelled `VALUE` by itself. The implemented signal workflow requires a model to pass the stored calibration policy on adequate permitted history before its independently generated prediction can be joined to a fresh compatible price. It persists model edge, EV, lower-bound EV, calibration, freshness, movement, bookmaker coverage, reasons, risks, and exact prediction/price/evaluation references. The underdog view ranks only positive-EV HOME or AWAY outcomes classified VALUE or WATCH; long odds alone never qualify.

Matchday, tax-aware arbitrage, scoreline-based bet builders, timestamp-valid stored-signal return backtests, and constrained bankroll research are implemented in the API and dashboard. Bet-builder legs are evaluated by scoreline intersection, not naive multiplication. Strategy replays keep price, prediction, result, cutoff, feature, and fingerprint provenance; bankroll simulations use settled observations and cap per-bet and daily exposure. Synthetic history validates software behavior only, CLV is unavailable without closing prices, and no model performance or profitability result is claimed.
